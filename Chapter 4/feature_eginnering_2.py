...
# ===============================================================
# 2) TRAIN-TEST SPLIT (✅ FIXED VERSION)
# ===============================================================

sensor_cols = ["acc_x", "acc_y", "acc_z", "gyr_x", "gyr_y", "gyr_z"]
categorical_cols = ["participant", "category"]
target_col = "label"

#  Use ALL features for splitting
X = df[sensor_cols + categorical_cols].copy()
...
X_train_raw = X_train[sensor_cols].copy()
X_test_raw  = X_test[sensor_cols].copy()
...
# IMPORTANT FIX: use original sensor columns, not time_cols
...
X_train_cat = pd.get_dummies(X_train_cat, prefix=categorical_cols,drop_first=True).astype(int)
X_test_cat  = pd.get_dummies(X_test_cat, prefix=categorical_cols,drop_first=True).astype(int)
...
X_train = pd.concat(
    [train_df[time_cols + fourier_cols].reset_index(drop=True),
     X_train_cat.reset_index(drop=True)],
    axis=1
)

X_test = pd.concat(
    [test_df[time_cols + fourier_cols].reset_index(drop=True),
