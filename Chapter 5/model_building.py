...
df_plot = X_train.sample(n=2000, random_state=42)

# Nice and clean pairplot
sns.pairplot(
    df_plot,
    hue='label',
    vars=["acc_x_mean", "acc_y_std", "gyr_x_entropy", "acc_r"],
    diag_kind='kde',
    corner=True,
    palette='tab10',
    plot_kws={'alpha': 0.7, 's': 40},   # Makes points slightly transparent + bigger
    diag_kws={'fill': True}
)

plt.suptitle("Pairplot of Selected Sensor Features by Label",
...
# ===============================================================
# 6) DEEP LEARNING PREP (FINAL)
# ==============================================================

# Convert to float32 (important for DL performance)
X_train_dl = X_train_classical.astype(np.float32)
X_test_dl  = X_test_classical.astype(np.float32)

# Encode targets
y_train_dl = to_categorical(y_train_classical)
y_test_dl  = to_categorical(y_test_classical)

# Create validation split (NO DATA LEAKAGE)
